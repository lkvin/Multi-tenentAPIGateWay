...
export const loginUser = async (payload: userPayload): Promise<userRespond> => {
    const existedUser = await prisma.user.findUnique({ where: { email: payload.email } });
    if (!existedUser) {
        //test first, in login email not found or wrong pass will retrun the same "wrong credential"
        throw new Error("User not found")
    }
    const verifyPw = await verifyPassword(payload.password, existedUser.hashPassword);
    if (!verifyPw) {
        throw new Error("Wrong email/password")
    }
    const { hashPassword, ...returnUser } = existedUser;
    const JWTtoken = await genToken({ userId: existedUser.id, email: existedUser.email });
    return {
        user: returnUser,
        token: JWTtoken
    };
